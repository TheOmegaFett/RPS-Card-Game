...
function GameOver({ playerScore, aiScore, onPlayAgain, onBackToMenu }) {
  const getResult = () => {
    if (playerScore > aiScore) {
      return { message: "You Win!", emoji: "🎉", type: "win" };
    } else if (aiScore > playerScore) {
      return { message: "AI Wins!", emoji: "😢", type: "loss" };
    } else {
      return { message: "It's a Draw!", emoji: "🤝", type: "draw" };
    }
  };
  ...
  return (
    <div className="game-over-overlay">
      <div className="game-over-card" data-result={result.type}>
        <div className="game-over-emoji">{result.emoji}</div>
        <h1 className="game-over-title">{result.message}</h1>
